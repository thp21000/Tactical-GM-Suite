...
function imageFrame(
  ...
  return buildImage(
    ...
    { dpi, offset: { x: logicalWidth / 2, y: logicalHeight / 2 } },
  )
    .id(id)
...
function iconTile(
  ...
  const result: Item[] = [unitFrameItem(ctx, `${baseId}-tile`, position, size, !active)];
  const icon = iconItem(
    ctx,
    `${baseId}-icon`,
    item,
...
export async function createOrUpdateTokenOverlay(token: StatTrackedToken): Promise<StatOverlayObrSyncResult> {
  ...
  try {
    ...
    const counts = Object.fromEntries(audiences.map(({ visibility, payload }) => [visibility, payload.itemCount])) as Record<StatTrackerVisibility, number>;
    const message = `Public ${counts.public} · Privé ${counts.private} · MJ ${counts.gm}`;
    if (created > 0) return createResult(action, deleted > 0 ? "updated" : "created", `Stat Dock mis à jour · ${message}`, { sourceItemId });
...
export async function deleteTokenOverlay(token: StatTrackedToken): Promise<StatOverlayObrSyncResult> {
  ...
  try {
    ...
    return createResult(action, deleted > 0 ? "deleted" : "not-found", deleted > 0 ? "Stat Dock supprimé." : "Aucun Stat Dock trouvé.", { sourceItemId });
  } catch (error) {
