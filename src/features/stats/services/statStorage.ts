...
import { createEmptyStatTrackerState, createTrackedToken, normalizeTokenType } from "./statTokens";

type StatMetadata = Metadata & { [ROOM_METADATA_KEYS.STAT_TRACKER_STATE]?: unknown };
type LegacyResource = { id?: string; name?: string; current?: number; max?: number };
type LegacyEntity = {
  id?: string;
  sourceItemId?: string;
  name?: string;
...
type LegacyState = { id?: string; entities?: LegacyEntity[]; createdAt?: string; updatedAt?: string };
...
function migrateLegacyEntity(entity: LegacyEntity) {
  ...
  if (typeof entity.currentHp === "number" || typeof entity.maxHp === "number") {
    trackers.push(createTracker({ name: "PV", visualType: "bar", iconId: "heart", current: entity.currentHp ?? entity.maxHp ?? 0, max: entity.maxHp ?? entity.currentHp ?? 0, visibility: "gm" }));
  }
  if (typeof entity.tempHp === "number" && entity.tempHp > 0) {
    trackers.push(createTracker({ name: "PVT", visualType: "counter", iconId: "temp-heart", value: entity.tempHp, visibility: "gm" }));
  }
  if (typeof entity.armorClass === "number") {
    trackers.push(createTracker({ name: "CA", visualType: "readonly", iconId: "armor", value: entity.armorClass, visibility: "gm" }));
  }
  for (const resource of entity.resources ?? []) {
    ...
    trackers.push(createTracker({ name: resource.name || "Ressource", visualType: max && max > 1 ? "bar" : "counter", iconId: "counter", current, max, value: current, visibility: "gm" }));
  }
  for (const condition of entity.conditions ?? []) {
    if (condition.name) trackers.push(createTracker({ name: condition.name, visualType: "icon", iconId: "toggle", visibility: "gm" }));
  }

  return { ...token, trackers, createdAt: entity.createdAt ?? token.createdAt, updatedAt: entity.updatedAt ?? token.updatedAt };
}
...
export function normalizeStatTrackerState(value: unknown): StatTrackerState {
  if (isV21State(value)) {
    ...
    return {
      ...
      tokens: value.tokens.map((token) => ({ ...token, trackers: Array.isArray(token.trackers) ? token.trackers : [] })),
      groups: value.groups,
...
export async function readStatTrackerState(): Promise<StatTrackerState> {
  ...
  return normalizeStatTrackerState(readJson<unknown>(STORAGE_KEYS.STAT_TRACKER_STATE, createEmptyStatTrackerState()));
}
...
export function subscribeToStatTrackerState(onChange: (state: StatTrackerState) => void): () => void {
